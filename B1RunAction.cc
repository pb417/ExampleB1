...
void B1RunAction::EndOfRunAction(const G4Run* run)
{
  ...
  // Merge accumulables 
  G4AccumulableManager* accumulableManager = G4AccumulableManager::Instance();
  G4VAccumulable* Energia = accumulableManager->GetAccumulable(1);
  ...
  G4int j = 0;
  G4cout << "EVENT                    E deposited (picoGy) " << G4endl;

  for (j = 0; j < N; j++)
    {
      //Evector[j]=Evector[j]/1.6e-4; // Conversion to eV/gram
...
{
  G4cout << "I add this number to the vector = " << edep << G4endl;
  Evector[i] = edep;
  i++;
}
